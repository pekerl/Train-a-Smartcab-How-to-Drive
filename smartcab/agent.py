...
class LearningAgent(Agent):
    ...
    def __init__(self, env, learning=True, epsilon=1, alpha=0.95):
    ...
    def reset(self, destination=None, testing=False):
        ...
        if testing is True :
        	...
        	self.epsilon = (math.pow(self.alpha,self.n_trials))  #optimized learning
    ...
    def choose_action(self, state):
        ...
        #   Otherwise, choose an action with the highest Q-value for the current state
        	...
        	if Qtable.has_key(self.state) :
        		...
        		Qtable.update({self.state : {None : self.Q_init, 'forward' : self.Q_init, 'left' : self.Q_init, 'right' : self.Q_init}})
        		action = random.choice(self.valid_actions)
    ...
    def learn(self, state, action, reward):
        ...
        if self.learning is True:
        	if self.steps_counter > 0 :
				Q_new = Qtable[self.state_before][self.action_before]
				Q_new = (1-self.alpha) * Q_new + self.alpha * self.reward_before
				Qtable[self.state_before][self.action_before] = Q_new
				self.Q = Qtable
        	
        self.state_before = self.state
        self.action_before = action
        self.reward_before = reward
...
def run():
    ...
    sim = Simulator(env, log_metrics=True, update_delay = 0.000001, optimized = True)
